...
| CH-9 client-aware chat header + deep-link                            | 2026-05-08 | Added client-book chat links, linked client ids on mirrored chat summaries, route query auto-open for `/communications/chats?clientId=...`, and a thread header context card with client job totals/last/next context.                                                                                                                 |
...
- Added automatic Quo contact sync on entry create/update and client profile updates (best-effort, non-blocking for entry writes).
- Added explicit link-management endpoints:
...
- Added unlinked inbound queue + resolve flow:
  - `GET /communications/chats/conversations/unlinked`
  - `POST /communications/chats/conversations/:conversationId/resolve`
- Added safe link reassignment behavior so a Quo contact can move between canonical client IDs without duplicate-link deadlocks.
...
- **Delivered**
  - Added durable `chat_quo_contacts` and `chat_quo_contact_phones` cache tables with lookup/stats helpers.
  - Chat sync hydrates names from the local contact cache first and only refreshes Quo contacts when the cache is stale/missing.
  - Full contact refreshes mark contacts missing from Quo as stale so deleted/removed contacts stop showing as valid cached matches.
  - Sync responses now expose contact-cache reuse/freshness stats so the Chats UI can explain whether it scanned Quo or reused local contact data.
...
- **Status**: Completed on 2026-05-08.
- **Delivered**
  - Client detail drawers expose an **Open chat** action that routes to `/communications/chats?clientId=...`.
  - Mirrored conversation summaries now include `linkedClientId` from the local chat/client link table.
  - Chats auto-opens the matching linked thread when a `clientId` query param is present.
  - Active chat threads show a client context card with name, address, total jobs, last job, next job, and a quick jump back to Client Book.
