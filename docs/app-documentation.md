...
- **Employees module**:
  ...
  - Planned next endpoints for execution tracking: per-run employee early clock-out and continuity-run creation for completed jobs (advanced-only flow).
...
- Execute the remaining job-execution tracking slices: manual mid-job clock-out and completed-job continuity workflow.
