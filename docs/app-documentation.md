...
- **Manage Employees UX refinement (UX-1/UX-2/UX-5/UX-6 in progress)**: the shell now includes a workflow guidance strip and a roster-first workspace-mode jump row (`Roster focus`, `Clock board`, `Profile editor`, `Hours editor`, `History timeline`, `Assignment readiness`) that progressively reveals one deep operation panel at a time to reduce first-screen complexity; workspace loading/error/empty messaging plus button/chip emphasis now uses shared styling tokens for consistency with Start Next Job, and form validation now exposes required semantics + assertive live-region errors.
- **Start Next Job board (ME-8 + ME-21)**: `/jobs/start` lazy-loads `StartNextJobShellComponent` with `StartNextJobFacade`. It consumes `/api/employees/readiness` + `/api/employees/history`, supports crew selection, validates scheduling conflicts against upcoming windows, persists confirmed assignments through `/api/employees/assignments/start-next-job`, and supports scheduled-history lifecycle actions (edit schedule, complete, cancel, reassign, plus bulk complete/cancel) directly from the board. Mutation flows now reconcile history/readiness optimistically with rollback on failure; the summary panel exposes selected-crew analytics; operators can export those analytics/history rows to CSV; Step 2 date-range filters scope both displayed metrics and exported data; per-employee trend cards provide crew-level drill-down; route-level variance cards surface heavy/light sites; and cross-run trend cards (with 7/30/90/custom windows) show schedule quality over time in the same pane.
- **Start Next Job UX refinement (UX-3/UX-4/UX-5/UX-6 in progress)**: in addition to the workflow strip + jump row, step buttons now lock until prerequisites are met, status rows call out missing inputs, panel-level "Continue" CTAs guide operators through `select crew -> set schedule -> review/save`, analytics now default to KPI-only with a Show/Hide details toggle for heavy trend cards, workspace state/button/chip feedback uses shared styling tokens, and controls now expose ARIA pressed/expanded + required/validation semantics for clearer keyboard/screen-reader flow.
