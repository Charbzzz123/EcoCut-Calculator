...
- **Manage Employees UX refinement (UX-1 in progress)**: the shell now includes a workflow guidance strip plus a quick-jump row (`Roster`, `Clock`, `Profile`, `Hours`, `History`, `Readiness`) so operators can follow a clear order (`filter/search -> choose operation -> save/verify`) before opening deeper editor panels.
...
- **Start Next Job UX refinement (UX-1 in progress)**: a workflow guidance strip plus step jump row now appear above the draft form to anchor the primary path (`select crew -> set schedule -> review/save`) before secondary analytics.
