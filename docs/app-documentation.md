...
- **Manage Employees UX refinement (UX-1/UX-2 in progress)**: the shell now includes a workflow guidance strip and a roster-first workspace-mode jump row (`Roster focus`, `Clock board`, `Profile editor`, `Hours editor`, `History timeline`, `Assignment readiness`) that progressively reveals one deep operation panel at a time to reduce first-screen complexity.
...
- **Start Next Job UX refinement (UX-3/UX-4 in progress)**: in addition to the workflow strip + jump row, step buttons now lock until prerequisites are met, status rows call out missing inputs, panel-level "Continue" CTAs guide operators through `select crew -> set schedule -> review/save`, and analytics now default to KPI-only with a Show/Hide details toggle for heavy trend cards.
