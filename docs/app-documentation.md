...
- **Start Next Job board (ME-8 + ME-21 + JX-1)**: `/jobs/start` lazy-loads `StartNextJobShellComponent` with `StartNextJobFacade`. It consumes `/api/employees/readiness` + `/api/employees/history`, supports crew selection, validates scheduling conflicts against upcoming windows, persists confirmed assignments through `/api/employees/assignments/start-next-job`, and supports scheduled-history lifecycle actions (edit schedule, complete, cancel, reassign, plus bulk complete/cancel) directly from the board. Step 2 now also consumes `/api/employees/job-options` so dispatch can link an assignment to an existing saved client job; linked selections auto-fill site/address/schedule and persist `jobEntryId` into assignment history + hours records. The board now starts in a job-first flow (`Job -> Crew -> Review`) with explicit required job mode selection (`linked` or `manual`) before crew-step progression. Mutation flows now reconcile history/readiness optimistically with rollback on failure; the summary panel exposes selected-crew analytics; operators can export those analytics/history rows to CSV; Step 2 date-range filters scope both displayed metrics and exported data; per-employee trend cards provide crew-level drill-down; route-level variance cards surface heavy/light sites; and cross-run trend cards (with 7/30/90/custom windows) show schedule quality over time in the same pane.
- **Start Next Job execution-tracking upgrade (planned)**:
  - Shift to a job-first flow: choose linked client job first, then assign crew.
  - Add live run lifecycle endpoints for `start` / `end` operations instead of relying only on planned schedule edits.
  - Add per-employee mid-job clock-out action for early departures; ending a run auto-clocks out remaining crew.
  - Support multiple active runs in parallel while preventing one employee from being active on more than one run.
  - Keep completed-job reuse behind an advanced `manual continuity` path that appends a linked follow-up segment for issue tracking.
- **Start Next Job UX refinement (UX-3/UX-4/UX-5/UX-6/UX-11/UX-12 complete)**: the board now uses a guided top strip with step locks + readiness messaging while rendering one major panel at a time (`Crew`, `Draft`, `Review`, or `Scheduled history`) to reduce overload. Draft and review sections include explicit Back/Continue actions, advanced draft controls remain collapsible, analytics stay KPI-first with optional detailed trends, and scheduled-history lifecycle actions are isolated into their own step. Workspace state/button/chip feedback uses shared styling tokens, and controls expose ARIA pressed/expanded + required/validation semantics for clearer keyboard/screen-reader flow.
...
- **Employees module**:
  ...
  - `POST /employees/assignments/start-next-job` - persist selected Start Next Job crew into scheduled history entries plus assignment-source hours rows; accepts optional `jobEntryId` to bind assignment metadata to a saved client job.
  ...
  - Planned next endpoints for execution tracking: run start/end lifecycle operations, per-run employee early clock-out, and continuity-run creation for completed jobs (advanced-only flow).
...
- Execute the job-execution tracking upgrade: job-first draft flow, active run start/end controls, manual mid-job clock-out, and completed-job continuity workflow.
