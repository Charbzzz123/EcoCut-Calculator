...
- **Start Next Job UX refinement (UX-3 in progress)**: in addition to the workflow strip + jump row, step buttons now lock until prerequisites are met, status rows call out missing inputs, and panel-level "Continue" CTAs guide operators through `select crew -> set schedule -> review/save`.
