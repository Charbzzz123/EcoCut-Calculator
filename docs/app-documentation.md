...
- **Manage Employees UX refinement (UX-1/UX-2/UX-5/UX-6 complete)**: the shell now includes a workflow guidance strip, a roster-first workspace-mode jump row (`Roster focus`, `Clock board`, `Profile editor`, `Hours editor`, `History timeline`, `Assignment readiness`), and explicit toolbar grouping for `Primary controls` vs `Advanced controls`; workspace loading/error/empty messaging plus button/chip emphasis use shared styling tokens for consistency with Start Next Job, and form validation exposes required semantics + assertive live-region errors.
