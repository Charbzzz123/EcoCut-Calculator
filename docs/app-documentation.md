...
- **Start Next Job board (ME-8 + ME-14)**: `/jobs/start` lazy-loads `StartNextJobShellComponent` with `StartNextJobFacade`. It consumes `/api/employees/readiness` + `/api/employees/history`, supports crew selection, validates scheduling conflicts against upcoming windows, persists confirmed assignments through `/api/employees/assignments/start-next-job`, and supports scheduled-history lifecycle actions (edit schedule, complete, cancel, reassign, plus bulk complete/cancel) directly from the board.
...
- Continue Start Next Job enhancements with deeper optimistic reconciliation and assignment analytics now that create/complete/edit/cancel/reassign plus bulk lifecycle flows are live.
