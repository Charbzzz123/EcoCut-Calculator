...
- Continue Start Next Job enhancements with deeper assignment performance drill-down (e.g., cross-run trend charts and seasonal route baselines) now that analytics, date-range filtering, per-employee trend cards, route-level variance cards, optimistic lifecycle reconciliation, and CSV export are live.
