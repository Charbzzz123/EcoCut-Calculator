...
- **Manage Employees workspace (ME-1/ME-2)**: `/employees/manage` now lazy-loads `ManageEmployeesShellComponent` with `EmployeesFacade` + `EmployeesDataService`. Current release includes roster retrieval, search, active/inactive filtering, summary metrics, and loading/error/empty states while preserving style/route contracts for upcoming profile/hours/history slices.
